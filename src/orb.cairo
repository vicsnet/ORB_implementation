...
// ERC721 interface
#[starknet::interface]
pub trait IERC_721<TContractState> {
...
#[starknet::interface]
pub trait IERC_165<TContractState> {
    fn suports_interfaace(self: @TContractState, interface_id: ByteArray) -> bool;
}

#[starknet::interface]
pub trait IERC721_token_receiver<TContractState> {
...
#[starknet::interface]
pub trait OrbInvocation<TContractState> {
...
#[starknet::interface]
pub trait IOrbTrait<TContractState> {
    fn get_total_supply(self: @TContractState) -> u256;

    fn token_balance_of(self: @TContractState, owner_: ContractAddress) -> u256;

    fn set_price(ref self: TContractState, price_: u256);

    fn start_orb(ref self: TContractState);
    ...
    fn extend_honored_until(ref self: TContractState, new_honored_until: u256);

    fn set_cool_down(ref self: TContractState, new_cooldown: u256, new_flagging_period: u256);

    fn set_clear_text_maximum_length(ref self: TContractState, new_clear_text: u256);
    ...
    fn buy_nonactive_orb(
        ref self: TContractState, token_id_: u256, amount_: u256, token_address_: ContractAddress
    );

    fn deposit(ref self: TContractState, amount_: u256, token_address_: ContractAddress);

    fn withdraw_all_fund(ref self: TContractState, token_address_: ContractAddress);

    fn withdraw_fund(ref self: TContractState, amount_: u256, token_address_: ContractAddress);

    fn relinquish(ref self: TContractState, token_id_: u256,);
    ...
    fn foreclose(ref self: TContractState, token_id_: u256);
...
pub mod ORB {
    ...
    #[derive(Drop, starknet::Event)]
    pub struct OathSwearing {
        #[key]
        pub oath_hash: ByteArray,
        pub honored_until: u256,
        pub response_period: u256
    }

    #[derive(Drop, starknet::Event)]
    pub struct HonoredUntilUpdate {
        #[key]
        pub previous_honored_until: u256,
        pub new_honored_until: u256,
    }
    #[derive(Drop, starknet::Event)]
    pub struct CooldownUpdate {
        #[key]
        pub previous_cooldown: u256,
        pub new_cooldown: u256,
        pub previous_flagging_period: u256,
        pub new_flagging_period: u256,
    }
    #[derive(Drop, starknet::Event)]
    pub struct CleartextMaximumLengthUpdate {
        #[key]
        pub previous_cleartext_maximum_length: u256,
        pub new_leartext_maximum_length: u256,
    }
    #[derive(Drop, starknet::Event)]
    pub struct BuyOrb {
        #[key]
        ...
        pub amount_: u256,
        pub fractioned_unit_: u256,
        pub token_id: u256
    }
    #[derive(Drop, starknet::Event)]
    pub struct NonActivePurchase {
        #[key]
    ...
    #[derive(Drop, starknet::Event)]
    pub struct OrbReliquish {
        #[key]
        pub orb_owner: ContractAddress,
        pub token_id: u256
    }
    #[derive(Drop, starknet::Event)]
    ...
    impl OrbTrait of super::IOrbTrait<ContractState> {
        ...
        fn start_orb(ref self: ContractState) {
            ...
            assert(
                self.honored_until.read() > 0 || current_time < self.honored_until.read(),
                'OATH_NOT_SWORN'
            );
        ...
        fn extend_honored_until(ref self: ContractState, new_honored_until: u256) {
            ...
            let is_owner = self.only_owner(caller);
            assert(is_owner == true, 'NOT_ORB_CREATOR');
            let previous_honor = self.honored_until.read();
            assert(self.honored_until.read() > 0, 'OATH_NOT_SWORN');
            assert(new_honored_until > previous_honor, 'HONORED_DATE_DISAPPROVED');

            self.honored_until.write(new_honored_until);
            self
                .emit(
                    HonoredUntilUpdate {
                        previous_honored_until: previous_honor,
        ...
        fn set_cool_down(ref self: ContractState, new_cooldown: u256, new_flagging_period: u256) {
            ...
            let previous_cooldown = self.cooldown.read();
            let previous_flagging_period = self.flagging_period.read();
            ...
            self
                .emit(
                    CooldownUpdate {
                        ...
                        previous_flagging_period: previous_flagging_period,
        ...
        fn buy_orb(
            ...
            self.last_token_id.write(new_id);
        ...
        fn buy_nonactive_orb(
            ...
            assert(amount_ >= purchased_price_, 'WRONG_AMOUNT');
        ...
        fn set_last_invocation_time(
            ref self: ContractState, token_id_: u256, owner_: ContractAddress
        ) {
